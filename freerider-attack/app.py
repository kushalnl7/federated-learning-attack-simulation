...
import sys
import os
from client import FlowerFreeRiderClient
...
def run_simulation(malicious_percentage):
    print(f"\n🚀 Running simulation with {malicious_percentage}% free-rider clients...")
    ...
    def client_fn(cid: str):
        ...
        return FlowerFreeRiderClient(model, train_loader, test_loader, DEVICE, is_malicious)
    ...
    end_time = time.time()
    print(f"⏱️ Simulation with {malicious_percentage}% free-riders completed in {end_time - start_time:.2f} seconds.")
...
if __name__ == "__main__":
    ...
    with open("results_freerider.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    plt.title("Model Robustness under Free-rider Attack")
    plt.xlabel("Free-rider Client Percentage")
    plt.ylabel("Final Global Test Accuracy")
    plt.grid(True)
    plt.savefig("accuracy_under_freerider_attack.png")
    # plt.show()

    print("\n✅ Free-rider attack simulation results saved successfully!")