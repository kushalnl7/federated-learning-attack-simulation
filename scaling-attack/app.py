...
import sys
import os
from client import FlowerScalingClient
...
def run_simulation(malicious_percentage):
    print(f"\n🚀 Running simulation with {malicious_percentage}% malicious clients (Scaling Attack)...")
    ...
    def client_fn(cid: str):
        ...
        return FlowerScalingClient(model, train_loader, test_loader, DEVICE, is_malicious)
...
if __name__ == "__main__":
    ...
    plt.title("Model Robustness under Scaling Attack")
    plt.xlabel("Malicious Client Percentage")
    plt.ylabel("Final Global Test Accuracy")
    plt.grid(True)
    plt.savefig("accuracy_under_scaling_attack.png")
    # plt.show()

    print("\n✅ Scaling attack simulation results saved successfully!")