...
class FlowerScalingClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if self.is_malicious:
            # Scale the parameters aggressively
            scale_factor = np.random.uniform(5, 10)  # Random scaling between 5x to 10x
            print(f"⚠️ Malicious client scaling updates by factor: {scale_factor:.2f}")
            params = [p * scale_factor for p in params]
