...
import sys
import os
from client import FlowerAttackClient
...
def run_simulation(malicious_percentage):
    ...
    def client_fn(cid: str):
        ...
        return FlowerAttackClient(model, train_loader, test_loader, DEVICE, is_malicious, attack_type="model_poisoning")
...
if __name__ == "__main__":
    ...
    # Save attack results
    with open("results_attack.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    plt.title("Model Robustness under Attack")
    plt.xlabel("Malicious Client Percentage")
    plt.ylabel("Final Global Test Accuracy")
    plt.grid(True)
    plt.savefig("accuracy_under_attack.png")
    # plt.show()

    print("\n✅ Attack simulation results saved successfully!")
